...
def load_df_data_v1():
    # global variable
    global  df
    df = pd.read_csv('app_user_keyword_association/dataset/udn_news_preprocessed.csv', sep='|')
...
def api_get_userkey_associate(request):
    ...
    all_paragraphs, same_paragraph = get_same_para(df_query, key, cond, k=30)
        ...
        for para in paragraphs:
            ...
            if cond == 'and':
                ...
                if any([re.search(kw, para) for kw in user_keywords]):
                    same_para.append(para)
    return same_para, same_para[0:k]
